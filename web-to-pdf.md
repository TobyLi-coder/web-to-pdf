...
将任意网页截图并导出为高质量 PDF，自动还原原始分页。

**支持场景：**
- Scribd / Studocu 等文档查看器 → 按原始 PDF 分页精确还原
- 普通文章 / 小说页面 → 自动移除广告和站点导航，A4 比例分页
- 任何网页 → 通用截图滚动模式兜底

**功能：**
- 自动读取 Chrome Cookie（免登录墙）
- 隐藏所有 fixed/sticky 导航栏、工具栏
- 识别并移除内联广告（类名 + 标准尺寸匹配）
- 截图完成后重新访问页面做内容一致性验证
- 浏览器在屏幕外后台运行，不遮挡当前工作
...
**用法：**
```
/web-to-pdf <URL> [输出目录] [文件名.pdf] [最大页数]
```

**示例：**
```
/web-to-pdf https://www.scribd.com/document/378841629/...
/web-to-pdf https://www.studocu.com/... ~/Documents/Study 笔记.pdf
/web-to-pdf http://kanunu8.com/book3/6425/44976.html ~/Desktop chapter1.pdf 5
```
...
- 参数 1（必须）= 目标网页完整 URL
- 参数 2（可选）= 输出目录，默认当前目录
- 参数 3（可选）= PDF 文件名（含 .pdf），默认从 URL 自动生成
- 参数 4（可选）= 最大页数（用于测试，0 = 不限制）

### 第 1 步：安装依赖

```bash
pip3 install -q playwright browser-cookie3 Pillow 2>/dev/null || true
python3 -m playwright install chromium --quiet 2>/dev/null || true
```

### 第 2 步：写出核心脚本并执行

将以下完整 Python 脚本写到 `/tmp/web_to_pdf.py`，然后执行：
```bash
python3 /tmp/web_to_pdf.py "<URL>" "<输出目录>" "<文件名.pdf>" <最大页数>
```
...
async def screenshot_pages(cookies, out_dir, label, max_pages=0):
    ...
    async with async_playwright() as p:
        ...
        if ads: print(f"[{label}] 移除内联广告 {ads} 个")
        ...
        if ads2: print(f"[{label}] 滚动后再次移除广告 {ads2} 个")
        ...
        if studocu and studocu['pageCount'] >= 2:
            pages_list = [
                {"top": studocu['docStart'] + i * studocu['pageH'],
                 "height": studocu['pageH'], "width": studocu['contentW'],
            ...
            if pages_list:
                ...
                if avg_h > 5000:
                    ...
                    mode = "dom"
                    print(f"[{label}] ✓ DOM 分页（{sel_css}）：{len(pages_list)} 页")
        ...
        if not pages_list:
            ...
            if content_info:
                print(f"[{label}] ✓ 主内容区：{content_info['selector']}，高度={content_info['height']}px")
                step = A4_PAGE_H
                n    = (content_info['height'] + step - 1) // step
                pages_list = [
                    {"top": content_info['top'] + i * step, "height": step,
                     "width": content_info['width'], "left": content_info['left']}
                    for i in range(n)
                ]
                last = content_info['height'] - (n-1) * step
                if n > 0 and last > 0:
                    pages_list[-1]['height'] = last
            else:
            ...
            sel_css = None
            mode = "scroll"
            print(f"[{label}] 滚动模式（A4，每页{A4_PAGE_H}px），共{len(pages_list)}段")

        hidden = await page.evaluate(HIDE_UI_JS, sel_css or "__none__")
        if hidden: print(f"[{label}] 隐藏 {hidden} 个 fixed/sticky UI")
            ...
            if s2 and s2['pageCount'] >= 2:
                pages_list = [
                    {"top": s2['docStart'] + i * s2['pageH'],
                     "height": s2['pageH'], "width": s2['contentW'], "left": s2['contentX']}
...
执行完成后告知用户 PDF 保存路径、页数、使用了哪种模式（studocu / dom / scroll）。
若验证失败，说明具体哪页差异过大，建议手动检查对应截图。

---

## 注意事项

- 需要系统已安装 **Python 3** 和 **pip3**
- 需要 **Google Chrome** 已安装（用于读取 Cookie）
- 首次运行自动安装 playwright、browser-cookie3、Pillow（约 100 MB）
- 浏览器窗口会在屏幕外启动（`--window-position=9999,0`），不影响当前工作
- 仅支持 **macOS**（Chrome Cookie 路径为 macOS 格式）
- 独立 Python 脚本位于：`~/Documents/10_Projects/10_ClaudeCode/40_WebToPDF/web_to_pdf.py`
